...
Primero las instituciones, de cualquier índole sean estatales, ongs, empresas, aportan información en forma de contenido a la plataforma. 
Segundo, esa informaciòn la organizan en caminos temáticos que llamamos paths o mapas, organizados por niveles con diferentes tipos de actividades interactivas. 
Tercero, los ciudadanos (usuarios de la plataforma) exploran los paths, y aprenden jugando a las actividades que plantearon las organizaciones.
Cuarto, lciudadanos, pueden publicar preguntas en un apartado puntual de la aplicacion (que llamamos pool de preguntas) para que las organizaciones se hagan eco de ellas para el armado de nuevo contenido. Además los usuarios pueden participar en la creacion de nuevo contenido para enriquecer la experiencia de los demas usuarios.
...
Y como último actor, tenemos a los desarrolladores, la plataforma está preparada para permitir a devs entusiastas que puedan proponer nuevos tipos de actividades para su uso en la plataforma, a estos tipos de actividades funcionales les llamamos "templates", que veremos mas adelante. 
...
Sin embargo, aún tuvimos puntos de encuentro además de las llamadas a los recursos de la api. Como por ejemplo la implementación del servidor de autenticacion en nuestro sistema.
...
La solucion a esto fue el desarrollo de una api de servicio para interactuar programaticamente desde nuestra api con keycloak. entonces los usuarios lo unico que ven de keycloak es la pagina de login, y luego la configuracion de la organizacion en la plataforma gestiona via api los roles.
...
Para el despliegue se configuro un Nginx en la vps para disponibilizar los servicios, en el diagrama se observan los accesos a la aplicaciones del front, tanto la nuestra como el administrador de keycloak. Además de esto nginx permite el acceso a las apis alojadas en los contenedores de docker ante cada peticion del cliente. 
17. Autenticacion - keycloak - fede  
El flujo de autenticación es el siguiente, el cliente obtiene la aplicacion frontend, esta solicita el token a keycloak, y luego usa ese token para las peticiones a la api. La api verifica el token de keycloak con la clave publica provista por el server y responde si es válido.
18. Frontend - fede 
...
PRIMERA PRUEBA 26 minutos con algunas perdidas de tiempo.
22. Demo- ambos 
