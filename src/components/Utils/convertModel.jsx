...
export default function convertModel(model) {
    //console.log("model en convert model", model, JSON.parse(model))
    const parsedModel = JSON.parse(model);
    const convertedModel = {};

    const convertNested = (obj, path = '') => {
        const result = {};
      //  console.log("convertNested function**********")
        Object.keys(obj).forEach(key => {
            ...
            } else if (Array.isArray(obj[key])) { // si es un arreglo mantengo el valor
                result[key] = obj[key];
            } else if (typeof obj[key] === 'object' && obj[key] !== null) {
                result[key] = convertNested(obj[key], currentPath); // si es un objeto lo sigo convirtiendo
            } else {
                result[key] = obj[key];//si es primitivo lo mantengo               
            }
        });
    ...
    Object.keys(parsedModel).forEach(key => {
       // console.log("parsedModel", key, typeof parsedModel[key], parsedModel[key])
       if (typeof parsedModel[key] === 'string' && parsedModel[key].startsWith('[') && parsedModel[key].endsWith(']')) { // si es un arreglo como string lo parseo
        ...
        } else if (Array.isArray(parsedModel[key])) { // si es un arreglo mantengo el valor
            convertedModel[key] = parsedModel[key];
            ...
            convertedModel[key] = convertNested(parsedModel[key], key);
                ...
                convertedModel[key] = parsedModel[key];// en caso de primitivos 
            }
    });
